<?php $this->layout('KebabDetailsLayout'); ?>
...
<div class="product-container">
    <h1><?= htmlspecialchars($kebab->getName()) ?></h1>
...
document.addEventListener('DOMContentLoaded', function() {  
    ...
    function getAllIngredients() {
        ...
        .then(data => {
            const kebabId = <?= json_encode($kebab->getIdKebab()) ?>;
            fetch(`http://www.kebab.com/apiPhp/api_kebab.php?id=${kebabId}`)
            .then(response => response.json())
            .then(kebabData => {
                console.log('ingredientes:', data);
                console.log('Ingredientes del kebab:', kebabData.ingredients);
                displayIngredients(data, kebabData.ingredients);
                calculateKebabPrice();
            })
            .catch((error) => {
                console.error('Error al obtener los ingredientes del kebab:', error);
            });
        })
        .catch((error) => {
    ...
    function displayIngredients(allIngredients, kebabIngredients) {
        ...
        if (!Array.isArray(kebabIngredients)) {
            console.error('kebabIngredients no es un array:', kebabIngredients);
            kebabIngredients = [];
        }

        const kebabIngredientIds = kebabIngredients.map(ing => ing.id);

        allIngredients.forEach(ingredient => {
            const ingredientElement = createIngredientElement(ingredient);
            
            if (kebabIngredientIds.includes(ingredient.idIngredient)) {
    ...
    function calculateKebabPrice() {
        const basePrice = parseFloat(<?= json_encode($kebab->getBasePrice()) ?>);
        const addedIngredients = document.querySelectorAll('#added-ingredients .ingredient-item');
        let totalPrice = basePrice;

        addedIngredients.forEach(ingredient => {
            totalPrice += parseFloat(ingredient.dataset.price) || 0;
        });
    ...
    const kebabId = <?= json_encode($kebab->getIdKebab()) ?>;
    const kebabName = <?= json_encode($kebab->getName()) ?>;
    ...
    const customKebab = {
        id: kebabId,
        name: kebabName,
        basePrice: <?= json_encode($kebab->getBasePrice()) ?>,
